...
class Grid:
    ...
    def show(self, save=False, title="maze"):
        self.__drawB()
        plt.show()
        plt.savefig(f"{title}.png")
        
    def __drawB(self):
        if os.listdir("./maze/") and self.__saveSteps:
            os.system("rm ./maze/*")
        
        maze = self.__str__()

        draw = self.__redraw(maze)
        ...
        plt.pcolormesh(draw, norm=self.norm, cmap=self.cmap)
    ...
    def progress(self):
        if not self.__saveSteps: return
        
        self.__step += 1
        self.saveFrame(self.__step)
        
    def createGif(self, title):
        self.__drawB()
        
        def bar(complete, title):
            bar = "█" * (complete) + "." * (10 - complete)
            os.system("clear")
            print(f"{title}\no~~|{bar}⫸  {complete * 10}%")
        
        if not self.__saveSteps: 
            print("Can't create gif")
            return
        
        for index, frame in enumerate(self.frames):
            plt.pcolormesh(frame, norm=self.norm, cmap=self.cmap)
            plt.savefig(f"./maze/maze_{index}.png")
            complete = (index +1 ) * 100 // self.__step
            if complete % 10 == 0:
                bar(complete // 10, "Creating images")
                
        
        with iio.get_writer(f"{title}.gif", mode="I") as writer:
            for step in range(1, self.__step):
                complete = step * 100 // self.__step
                if complete % 10 == 0:
                    bar(complete // 10, "Creating gif")
                img = iio.imread(glob.glob(f"./maze/*_{step}.png")[0])
